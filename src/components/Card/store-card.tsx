...
export const StoreCard = React.memo(({ item, onEdit, onDelete }: StoreCardProps) => {
    return (
        <Box
            bg="$white"
            p="$4"
        ...
        >
            <Box style={cardStyles.cardHeader}>
                ...
                <Box style={cardStyles.productBadge}>
                    <Text style={cardStyles.productBadgeText}>Total de produtos: {item.productCount}</Text>
                </Box>
            </Box>
            ...
            <Box style={cardStyles.cardActions}>
                <Button
                    ...
                    onPress={() => onEdit(item)}>
                    <Text style={[cardStyles.buttonSecondaryText]}>Editar</Text>
                </Button>

                <Button
                    style={[cardStyles.button, cardStyles.buttonPrimary]}
                    onPress={() => router.push(`/products/${item.id}` as RelativePathString)}
                >
                    <Text style={[cardStyles.buttonPrimaryText, cardStyles.buttonText]}>Produtos</Text>
                </Button>

                <Button
                    style={[cardStyles.button, cardStyles.buttonDelete]}
                    onPress={() => onDelete(item.id)}>
                    <Text style={[cardStyles.buttonDeleteText, cardStyles.buttonText]}>Deletar</Text>
                </Button>
            </Box>
        </Box>
    )
